    ...
    static std::string enc(std::string s, int k) {
        for (char& c : s) {
            if (isalpha(static_cast<unsigned char>(c))) {
                char b = isupper(static_cast<unsigned char>(c)) ? 'A' : 'a';
                c = static_cast<char>((c - b + k) % 26 + b);
            }
        }
        return s;
    }

    static std::string dec(const std::string& s, int k) {
        return enc(s, 26 - k);
    }
};
...
public:
    static std::vector<uint8_t> apply(const std::string& text,
        ...
        for (size_t i = 0; i < text.size(); i++) {
            out.push_back(static_cast<uint8_t>(text[i] ^ key[i % key.size()]));
        }
        return out;
    }
    ...
    static std::string recover(const std::vector<uint8_t>& ct,
        ...
        for (size_t i = 0; i < ct.size(); i++) {
            out += static_cast<char>(ct[i] ^ key[i % key.size()]);
        }
        return out;
    }
